...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/gen/genTest/">合同列表</a></li>
		<li class="active"><a href="${ctx}/gen/genTest/form?id=${genTable.id}&name=${genTable.name}">合同表<shiro:hasPermission name="gen:genTable:edit">${not empty genTable.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="gen:genTable:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<c:choose>
		<c:when test="${empty genTable.name}">
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTest/form" method="post" class="form-horizontal">
				<form:hidden path="id"/>
