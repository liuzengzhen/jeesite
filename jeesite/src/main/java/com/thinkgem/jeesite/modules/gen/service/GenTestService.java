...
import com.thinkgem.jeesite.modules.gen.dao.GenTestColumnDao;
import com.thinkgem.jeesite.modules.gen.dao.GenTestDao;
...
public class GenTestService extends BaseService {

	@Autowired
	private GenTableDao genTestDao;
	@Autowired
	private GenTableColumnDao genTestColumnDao;
	@Autowired
	private GenDataBaseDictDao genDataBaseDictDao;
	
	public GenTable get(String id) {
		GenTable genTable = genTestDao.get(id);
		GenTableColumn genTableColumn = new GenTableColumn();
		genTableColumn.setGenTable(new GenTable(genTable.getId()));
		genTable.setColumnList(genTestColumnDao.findList(genTableColumn));
		return genTable;
	}
	
	public Page<GenTable> find(Page<GenTable> page, GenTable genTable) {
		genTable.setPage(page);
		page.setList(genTestDao.findList(genTable));
		return page;
	}

	public List<GenTable> findAll() {
		return genTestDao.findAllList(new GenTable());
	}
	...
	public boolean checkTableName(String tableName){
		...
		List<GenTable> list = genTestDao.findList(genTable);
		return list.size() == 0;
	}
	...
	public void save(GenTable genTable) {
		if (StringUtils.isBlank(genTable.getId())){
			genTable.preInsert();
			genTestDao.insert(genTable);
		}else{
			genTable.preUpdate();
			genTestDao.update(genTable);
		}
		// 保存列
		for (GenTableColumn column : genTable.getColumnList()){
			...
			if (StringUtils.isBlank(column.getId())){
				column.preInsert();
				genTestColumnDao.insert(column);
			}else{
				column.preUpdate();
				genTestColumnDao.update(column);
			}
		}
	}
	...
	public void delete(GenTable genTable) {
		genTestDao.delete(genTable);
		genTestColumnDao.deleteByGenTableId(genTable.getId());
	}
